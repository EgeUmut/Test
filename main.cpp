...
int main(){
	...
	if(menu==1){//film ekleme menusu
		struct bilgi bilgi;
		// bu bir yorum satırıdır github test.
		...
		else if(menu==2){//s�ralama menusu 
		...
		/*		for(int i=0; i<SIZE; ++i){
				...
				if(dizi[b].yil<dizi[b+1].yil){  //�al��m�yor �imdilik....
					hold = dizi[b].yil;
					dizi[b] = dizi[b+1];
					dizi[b+1].yil = hold;
			}			
		...
		_fcloseall();
		remove("dosya.txt");//dosya.txt dosyas� silindi	
		rename("dosya1.txt", "dosya.txt");//Yeni a��lan dosya yeniden adland�r�ld�
			if (a == 0){
				printf("\nBu film bulunamadi!\n");
				}
		bitis();		
		...
		switch(secim3){
			case 1:
				...
				fscanf(fp, "%s", &bilgi.basoy);		//Dosyadan okunan di�er bilgiler
		...
		goto cikis;}//cikis sat�r�na gider
		
		else{	hata();				}
	cikis:
	return 0;
}//main fonksiyonu sonu
void hata(){//hatal� cevaplara kar�� verilen cevap
	int donus2;
	system("cls");
			menu1donus:
			printf("Hatali Giris\n");
...
void bitis(){//menu sonlar� i�in ana men�ye d�n�� ya da kapatmak i�in fonksiyon
	int donus4;
